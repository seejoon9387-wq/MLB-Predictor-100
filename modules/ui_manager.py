    ...
    def render_game_navbar(game_data_list):
        ...
        with cols[1]:
            ...
            for i, game in enumerate(page_games):
                ...
                with game_cols[i]:
                    st.markdown(f"""
                        <div style="background:#ffffff; border:1px solid #dddddd; border-radius:10px; padding:10px 5px; text-align:center; color:black; font-size:13px; box-shadow: 2px 2px 5px #eee;">
                            <div style="color:#555; font-size:10px; margin-bottom:5px;">{game['match_time']}</div>
                            <div style="display:flex; justify-content:space-between; padding:2px 4px;">
                                <span>{game['away_name']}</span> <b style="color:{away_color}; font-size:15px;">{game['away_score']}</b>
                            </div>
                            <div style="display:flex; justify-content:space-between; padding:2px 4px;">
                                <span>{game['home_name']}</span> <b style="color:{home_color}; font-size:15px;">{game['home_score']}</b>
                            </div>
                        </div>
